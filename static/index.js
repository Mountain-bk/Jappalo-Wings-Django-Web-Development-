...
if (timeRange[0]){
  ...
  if(date.getHours() < 11){
    ...
    checkMark.src = "../static/images/cross.png";
...
if(timeRange[0]){
  window.addEventListener("change", () =>{
    if(timeRange[0].options[timeRange[0].selectedIndex].text === "Select Time"){
      ...
      checkMark.src = "../static/images/check.png";
    }
  });
}
...
function showAddCartModal(selectedItem){
  ...
  modalImageContainer.innerHTML = "<img class='modal-image' src='../static/images/" + modalName + ".jpg'>";
...
//Display the cart every time when you load in desktop//
if(orderBeverage){
  animateDisplay();
}
...
function changePageOrClose(){
  if(alertMessage.innerHTML === "<p>Please select order type</p>"){
    document.location.href = "/order-type";
...
if(paymentBarBtn){
  paymentBarBtn.addEventListener("click", () =>{
    if(localStorage.getItem("shoppingCart") && localStorage.getItem("orderSetTime")){
      paymentBarBtn.setAttribute("href", "/payment");
...
if(paymentBtn){
  paymentBtn.addEventListener("click", () =>{
    if(localStorage.getItem("shoppingCart") && localStorage.getItem("orderSetTime")){
      paymentBtn.setAttribute("href", "/payment");
    }else if(localStorage.getItem("shoppingCart") === null){
...
if(paymentNavBtn){
  paymentNavBtn.addEventListener("click", () =>{
    if(localStorage.getItem("shoppingCart") && localStorage.getItem("orderSetTime")){
      paymentNavBtn.setAttribute("href", "/payment");
    }else if(localStorage.getItem("shoppingCart") === null){
